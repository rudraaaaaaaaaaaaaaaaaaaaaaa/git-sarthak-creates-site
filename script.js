...
(function(){
  ...
  window.addEventListener('orientationchange', fit);
    ...
    (function(){
      ...
      function updateParallax(){var scrollY=window.scrollY;var scrollDelta=scrollY-lastScrollY;ballBoost=Math.abs(scrollDelta)*18;var bgT=scrollY*-0.1,fgT=scrollY*-0.3,infoT=scrollY*-0.2;if(heroBg)heroBg.style.transform='translateY('+bgT+'px)';if(heroFg)heroFg.style.transform='translateY('+fgT+'px)';if(infoBox)infoBox.style.transform='translateY('+infoT+'px)';if(infoBox){var dvz=document.querySelector('.desktop-view');var zm=(dvz&&parseFloat(dvz.style.zoom))||1;var itop=infoBox.getBoundingClientRect().top;[[heroBg,bgT],[heroFg,fgT]].forEach(function(pp){var el=pp[0];if(!el)return;var im=el.querySelector('img');if(!im||!im.naturalWidth)return;var r=el.getBoundingClientRect();var sc=Math.min(r.width/im.naturalWidth,r.height/im.naturalHeight);var pb=r.top+im.naturalHeight*sc;var g=itop-pb;if(g>0.5)el.style.transform='translateY('+(pp[1]+g/zm)+'px)';});}var ib=Math.max(0,scrollY-600);if(bgGrid)bgGrid.style.transform='translateY('+(ib*0.08)+'px)';var cs=ib*-0.03;if(expSection)expSection.style.transform='translateY('+cs+'px)';if(workSection)workSection.style.transform='translateY('+(cs*1.2)+'px)';if(beyondSection)beyondSection.style.transform='translateY('+(cs*1.5)+'px)';var ft=document.querySelector('.desktop-view .footer');if(ft)ft.style.transform='translateY('+(cs*1.5)+'px)';if(servicesFan)servicesFan.style.transform='translateY('+(cs*0.8)+'px) scale(0.9)';lastScrollY=scrollY;ticking=false;}
